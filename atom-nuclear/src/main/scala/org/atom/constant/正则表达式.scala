...
object 正则表达式 {
  ...
  val 合法账号 = "^[a-zA-Z][a-zA-Z0-9_]{4,15}$" // 字母开头，允许5-16字节，允许字母数字下划线

  val 密码 = "^[a-zA-Z]\\w{5,17}$" // 以字母开头，长度在6~18之间，只能包含字母、数字和下划线

  val 强密码 = "^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).{8,10}$" // 必须包含大小写字母和数字的组合，不能使用特殊字符，长度在8-10之间

  val 日期格式 = "^\\d{4}-\\d{1,2}-\\d{1,2}"

  val 一年的12个月 = "^(0?[1-9]|1[0-2])$" // 01～09和1～12

  val 一个月的31天 = "^((0?[1-9])|((1|2)[0-9])|30|31)$" // 01～09和1～31

  val xml文件 = "^([a-zA-Z]+-?)+[a-zA-Z0-9]+\\\\.[x|X][m|M][l|L]$"

  val 双字节字符 = "[^\\x00-\\xff]" // (包括汉字在内，可以用来计算字符串的长度(一个双字节字符长度计2，ASCII字符计1))

  val 空白行 = "\\n\\s*\\r" // 可以用来删除空白行

  val 首尾空白字符 = "^\\s*|\\s*$或(^\\s*)|(\\s*$)" // 可以用来删除行首行尾的空白字符(包括空格、制表符、换页符等等)，非常有用的表达式

  val 腾讯QQ号码 = "[1-9][0-9]{4,}" // 腾讯QQ号从10000开始

  val IP地址 = "\\d+\\.\\d+\\.\\d+\\.\\d+" // 提取IP地址时有用
